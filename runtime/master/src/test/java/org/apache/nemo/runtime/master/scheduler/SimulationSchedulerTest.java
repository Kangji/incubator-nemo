...
public final class SimulationSchedulerTest {
  ...
  @Test(timeout = 10000)
  public void testSimulationFromMetrics() throws Exception {
    ...
    final PhysicalPlan physicalPlan2 =
      TestPlanGenerator.generatePhysicalPlan(TestPlanGenerator.PlanType.TwoVerticesJoined, false);
    scheduler.setTaskDurationEstimationMethod(SimulatedTaskExecutor.Type.AVERAGE_FROM_METRICS);
  ...
  @Test(timeout = 10000)
  public void testSimulationFromModel() throws Exception {
    final PhysicalPlan physicalPlan =
      TestPlanGenerator.generatePhysicalPlan(TestPlanGenerator.PlanType.TwoVerticesJoined, false);
    final JobMetric actualJobMetric =
      MetricStore.getStore().getOrCreateMetric(JobMetric.class, physicalPlan.getPlanId());
    actualJobMetric.setStageDAG(physicalPlan.getStageDAG());
    actualJobMetric.setIRDAG(TestPlanGenerator.getTwoVerticesJoinedDAG());

    final PhysicalPlan physicalPlan2 =
      TestPlanGenerator.generatePhysicalPlan(TestPlanGenerator.PlanType.TwoVerticesJoined, false);
    scheduler.setTaskDurationEstimationMethod(SimulatedTaskExecutor.Type.ANALYTIC_ESTIMATION);
    scheduler.schedulePlan(
      physicalPlan2,
      SCHEDULE_ATTEMPT_INDEX);
    ...
    resultingMetricStore.getMetricMap(TaskMetric.class).forEach((id, taskMetric) -> {
      assertTrue(0 <= ((TaskMetric) taskMetric).getTaskDuration());
      assertTrue(100000 > ((TaskMetric) taskMetric).getTaskDuration());
    });

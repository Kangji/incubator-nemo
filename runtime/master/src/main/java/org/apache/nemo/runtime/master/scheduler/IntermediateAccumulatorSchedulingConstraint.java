...
public final class IntermediateAccumulatorSchedulingConstraint implements SchedulingConstraint {
  ...
  public boolean testSchedulability(final ExecutorRepresenter executor, final Task task) {
    ...
    final HashSet<Pair<HashSet<String>, Pair<MutableInt, Integer>>> setsOfExecutorsAndParallelism
      = task.getPropertyValue(ShuffleExecutorSetProperty.class).get();
    return setsOfExecutorsAndParallelism.stream().anyMatch(p ->
      p.left().contains(executor.getExecutorId()) && p.right().left().intValue() > 0);
  }
}

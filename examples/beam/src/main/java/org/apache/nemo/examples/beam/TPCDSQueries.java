 ...
 * TPC-DS Schemas.
 */
public final class TPCDSQueries {

  /**
   * Private constructor.
   */
  private TPCDSQueries() {
  }
  ...
  public static final Schema GET_CUSTOMER_DS_SCHEMA =
    Schema.builder()
  ...
  public static final Schema CUSTOMER_DEMOGRAPHICS_SCHEMA =
    Schema.builder()
  ...
  public static final String QUERY4 =
    "with year_total as (\n"
      ...
      + "else null end\n"
      + "    and case when t_c_firstyear.year_total > 0 then t_c_secyear.year_total / t_c_firstyear.year_total else "
      + "null end\n"
  ...
  public static final String QUERY51 =
    " WITH web_v1 as (\n"
      ...
      + "       over (partition by ws_item_sk order by d_date rows between unbounded preceding and current row) "
      + "cume_sales\n"
      + " from web_sales\n"
      + "     ,date_dim\n"
      + " where ws_sold_date_sk=d_date_sk\n"
      ...
      + "       over (partition by ss_item_sk order by d_date rows between unbounded preceding and current row) "
      + "cume_sales\n"
      + " from store_sales\n"
      + "     ,date_dim\n"
      + " where ss_sold_date_sk=d_date_sk\n"
      ...
      + "      ,store_sales\n"
      + "      ,max(web_sales)\n"
      + "          over (partition by item_sk order by d_date rows between unbounded preceding and current row) "
      + "web_cumulative\n"
      + "      ,max(store_sales)\n"
      + "          over (partition by item_sk order by d_date rows between unbounded preceding and current row) "
      + "store_cumulative\n"
  ...
  public static final String QUERY76 =
    "select  channel, col_name, d_year, d_qoy, i_category, COUNT(*) sales_cnt, SUM(ext_sales_price) sales_amt FROM (\n"
      + "         SELECT 'store' as channel, 'ss_promo_sk' col_name, d_year, d_qoy, i_category, ss_ext_sales_price "
      + "ext_sales_price\n"
      ...
      + "         UNION ALL\n"
      + "         SELECT 'web' as channel, 'ws_bill_addr_sk' col_name, d_year, d_qoy, i_category, ws_ext_sales_price "
      + "ext_sales_price\n"
      ...
      + "         UNION ALL\n"
      + "         SELECT 'catalog' as channel, 'cs_bill_addr_sk' col_name, d_year, d_qoy, i_category,"
